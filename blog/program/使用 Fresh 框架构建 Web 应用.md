...
image: https://img.mlishu.cn/link-maker.png
---
...
![](https://img.mlishu.cn/image_jSRfPu966v.png)
...
![](https://img.mlishu.cn/image_v73eXB47yI.png)
...
核心代码在 [`routes\api\link.ts`](https://github.com/mlishu/link-maker/blob/main/routes/api/link.ts) 下，将会生成 `/api/link` 接口，例如访问 [https://link-maker.deno.dev/api/link?q=https://mlishu.cn](https://link-maker.deno.dev/api/link?q=https://mlishu.cn 'https://link-maker.deno.dev/api/link?q=https://mlishu.cn') 你就可以得到如下 json 数据

```json
{
  "title": "璃殊的小站",
  "description": "Blog",
  "image": "https://mlishu.cn/img/logo.png",
  "url": "https://mlishu.cn"
}
```
...
至于说为何要用 query 而不是用 param，主要是因为 url 的缘故，比如说 `/api/link/https://mlishu.cn` 这个链接，这时 param 是解析不出 `https://mlishu.cn` 完整 url 的，除非 url 编码，但这对使用者来说就不是很好，于是就舍弃了 param 参数的方案。
...
![](https://img.mlishu.cn/image_CYOAgv6IGe.png)
