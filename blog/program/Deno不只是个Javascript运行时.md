...
image: https://img.mlishu.cn/202312270248371.png
---
...
```bash
deno run https://deno.mlishu.cn/main.ts
```

在第一次使用时下载并缓存代码，你可以通过

```bash
deno info http://deno.mlishu.cn/main.ts
```

来查看文件信息，如下

![](https://img.mlishu.cn/image_deb0_lGYRA.png)
...
你可以通过 [https://mlishu.deno.dev/](https://mlishu.deno.dev/ 'https://mlishu.deno.dev/') 来访问我使用 deno Deploy 所创建的一个在线项目。将会输出一个`Hello World!` 的页面。
