...
-- 修改用户名与命名
UPDATE `user` SET `username`='mlishu',`password`='a12345678` WHERE id = 1
```
...
![img](https://img.mlishu.cn/20201009150524563.png)
...
SELECT LOWER('mlishu') --到小写
SELECT UPPER('mlishu') --到大写
```
...
CREATE USER mlishu IDENTIFIED BY '123456'

-- 修改密码 (修改当前用户)
SET PASSWORD = PASSWORD('a123456')

-- 修改密码 (修改指定用户)
SET PASSWORD FOR mlishu= PASSWORD('a123456')

-- 重命名
RENAME USER mlishu TO zeyu

-- 用户授权
-- CRANT ALL PRIVILEGES ON 库.表 TO 用户
-- ALL PRIVILEGES 除了给别人授权(GRANT),其他权限都有
CRANT ALL PRIVILEGES ON *.* TO mlishu

-- 查看权限
SHOW GRANTS FOR mlishu
SHOW GRANTS FOR root@localhost
-- root 的权限 GRANT PROXY ON ''@'' TO 'root'@'localhost' WITH GRANT OPTION

-- 撤销权限 REVOKE 权限 ON 库.表 FROM 用户
REVOKE ALL PRIVILEGES ON *.* FROM mlishu
...
![image-20210102213536190](https://img.mlishu.cn/image-20210102213536190.png)
